...
adminRouter.put('/settings', async (req, res) => {
  try {
    const { name, description, logo_url, phone, address, working_hours, payment_info } = req.body;

    if (!name || !name.trim()) {
      return res.status(400).json({ error: "Oshxona nomi majburiy" });
    }
    ...
    console.error(err);
    res.status(500).json({ error: err.message || 'Sozlamalarni yangilashda xatolik' });
  }
});
